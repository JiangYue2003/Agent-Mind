...
class MCPToolManager:
    ...
    async def rewrite_query(self, query: str, n: int = 3, trace: Any = None) -> List[str]:
        ...
        """
        prompt = f"""你是客服知识库检索改写助手。请将用户查询改写为 {n} 个用于检索的短子查询。

要求：
1. 保持原始语义，不要引入原问题中没有的新条件、新流程或新主题。
2. 保留订单号、金额、日期、制度名、版本号等关键实体；不要遗漏这些约束。
3. 优先输出贴近原问题的同义表达、关键槽位展开或时效/规则/流程中真正相关的角度。
4. 如果原问题已经清晰，不要为了凑数而过度改写，可以输出与原问非常接近的表达。
5. 子查询要简短、可检索，不要输出解释性句子，不要输出无关扩展问题。
